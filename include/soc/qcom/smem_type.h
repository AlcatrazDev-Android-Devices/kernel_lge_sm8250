...
$Header: //components/rel/core.mpss/8.0.c17/api/mproc/smem_type.h#1 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
09/12/17   ss      Adding SMEM_WLAN_CONFIG by duplicating 
                   SMEM_RESERVED_SMEM_SLOW_CLOCK_SYNC
02/09/17   vr      SMEM Version 0x000C0000 changes.
...
12/09/14   db      Added smem types for SMEM XPORT
07/22/14   bc      Add sensor smem_host_type and for sensor smp2p item
...
06/18/13   pv      Renaming a reserved SMEM item as it is used on A-family by, 
				           power team for notification of apps power status.
...
typedef enum
{
  ...
  SMEM_CDSP         =  5,                     /**< CDSP processor */
  SMEM_RPM          =  6,                     /**< RPM processor */
  SMEM_TZ           =  7,                     /**< TZ processor */
  SMEM_SPSS         =  8,                     /**< Secure processor */
  SMEM_HYP          =  9,                     /**< Hypervisor */
  SMEM_NPU			= 10,
  SMEM_SPSS_SP		= 11,  					  /**< SPSS Host that shares partition with TZ*/
  SMEM_NUM_HOSTS    = 12,                     /**< Max number of host in target */
  ...
  SMEM_SPSS_NONSP   =  SMEM_SPSS,         /**< SPSS host that shares partition with HLOS */
  SMEM_NSP       = SMEM_CDSP,
