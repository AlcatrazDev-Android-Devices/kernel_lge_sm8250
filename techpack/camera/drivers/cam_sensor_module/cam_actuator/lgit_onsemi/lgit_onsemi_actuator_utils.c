		...
		struct cam_actuator_ctrl_t *a_ctrl)
{
    ...
    switch(a_ctrl->soc_info.index)
    {
        ...
        case 1:
            if (REG_ADDR_SOC_1 != reg_addr) { return true; } break;
        case 3:
...
extern void actuator_create_sysfs(struct cam_actuator_ctrl_t *a_ctrl) {
	struct device*  camera_act_hall_dev = NULL;
    //static struct class *class_actuator = NULL;

    CAM_INFO(CAM_ACTUATOR, "actuator_create_sysfs");

	if(!a_ctrl->camera_class) {
		a_ctrl->camera_class = get_camera_class();

		if(a_ctrl->soc_info.index == 0 && a_ctrl->camera_class != NULL) {
            CAM_INFO(CAM_ACTUATOR, "soc_info.index = [ %d ], Create act_normal_hall_class!",
		...
		if(a_ctrl->soc_info.index == 1 && a_ctrl->camera_class != NULL) {
            CAM_INFO(CAM_ACTUATOR, "soc_info.index = [ %d ], Create act_vt_hall_class!",
            	a_ctrl->soc_info.index);
			camera_act_hall_dev   = device_create(a_ctrl->camera_class, NULL,
		...
		if(a_ctrl->soc_info.index == 3 && a_ctrl->camera_class != NULL) {
            CAM_INFO(CAM_ACTUATOR, "soc_info.index = [ %d ], Create act_tele_hall_class!",
            	a_ctrl->soc_info.index);
			camera_act_hall_dev   = device_create(a_ctrl->camera_class, NULL,
