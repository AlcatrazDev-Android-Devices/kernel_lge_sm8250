...
#include "lgit_s5kgw1_onsemi_ois.h"
#include "onsemi_ois.h"
#include "LC898124EP3_Code_9_1_1_0.h"
#include "LC898124EP3_Code_9_1_1_1.h"
#include <linux/device.h>
#include <linux/spinlock.h>
#include <asm/arch_timer.h>

#define LAST_UPDATE "20-01-16, OIS LC898124EP3"
...
#define E2P_SID                 (0xA2)
#define MODULE_FW_VER_ADDR      (0xAC8)

//#define GYROSELECT              (0x00)
//#define HALL_LIMIT              (1082)

#define PROPERTY_VALUE_MAX      (92)

#define OIS_HALL_SEQ_ADDR       (0xE001)
#define OIS_HALL_X_ADDR         (0x0268)
#define OIS_HALL_Y_ADDR         (0x026C)
#define OIS_GYRO_ADDR           (0xE002)
#define OIS_HALL_X_OFFSET       (0x01C0)
#define OIS_HALL_Y_OFFSET       (0x0214)
...
int lgit_s5kgw1_onsemi_ois_poll_ready(int limit)
{
...
static int msm_startGyroThread(struct cam_ois_ctrl_t *o_ctrl)
{
	...
	// Read Gyro Gain before thread start
	ois_i2c_read_seq(OIS_GYRO_GAIN_X, gyro_gain_x, 2);
	o_ctrl->gyro_gain_x = (gyro_gain_x[0] << 8 | gyro_gain_x[1]);
	ois_i2c_read_seq(OIS_GYRO_GAIN_Y, gyro_gain_y, 2);
	o_ctrl->gyro_gain_y = (gyro_gain_y[0] << 8 | gyro_gain_y[1]);
    ...
    CAM_ERR(CAM_OIS, " gain %d %d",
        o_ctrl->gyro_gain_x, o_ctrl->gyro_gain_y);
...
//TRACE("DM Start \n" );
	...
	if (Remainder != 0 )
	{
		CntWrt( (uint8_t*)DataDM, (uint8_t)Remainder );  // I2Caddresss 1Byte.
	}
		DataDM += Remainder;
...
if(Dspcode->ActType == ACT_TIMEVCM )
TRACE("actuator type : TIMEVCM\n");
...
const DOWNLOAD_TBL DTbl[] = {
 {0x0101, LC898124EP3_PM_9_1_1_0, LC898124EP3_PMSize_9_1_1_0, (uint32_t)((uint32_t)LC898124EP3_PMCheckSum_9_1_1_0 + (uint32_t)LC898124EP3_DMA_CheckSum_9_1_1_0 + (uint32_t)LC898124EP3_DMB_CheckSum_9_1_1_0), LC898124EP3_DM_9_1_1_0, LC898124EP3_DMA_ByteSize_9_1_1_0 , LC898124EP3_DMB_ByteSize_9_1_1_0 },
 {0x8101, LC898124EP3_PM_9_1_1_1, LC898124EP3_PMSize_9_1_1_1, (uint32_t)((uint32_t)LC898124EP3_PMCheckSum_9_1_1_1 + (uint32_t)LC898124EP3_DMA_CheckSum_9_1_1_1 + (uint32_t)LC898124EP3_DMB_CheckSum_9_1_1_1), LC898124EP3_DM_9_1_1_1, LC898124EP3_DMA_ByteSize_9_1_1_1 , LC898124EP3_DMB_ByteSize_9_1_1_1 },
 {0xFFFF, (void*)0, 0, 0, (void*)0 ,0 ,0 }
};
//LC898124EP3_PM_9_1_1_0 : Use LGE
//LC898124EP3_DM_9_1_1_1 : Only Use LGIT

int32_t lgit_s5kgw1_init_set_onsemi_ois(struct cam_ois_ctrl_t *o_ctrl)
{
	int32_t rc = OIS_INIT;
	uint8_t ActSelect = 0;
	uint16_t m_fw_ver = 0;
	...
	CAM_ERR(CAM_OIS, "Enter, %s, S_VER 0x%x M_VER 0x%x", LAST_UPDATE, FW_VER, m_fw_ver);
	...
	// Gyro Initial
	RamWrite32A( 0xF01E, 0x6B800000 ) ;
	WitTim(70);
	RamWrite32A( 0xF01E, 0x6B010000 ) ;
	WitTim(50);
	RamWrite32A( 0xF01E, 0x1B100000 ) ;
	WitTim(5);
	RamWrite32A( 0xF01F, 0x00000001 ) ;
...
//		usleep_range(200 * 1000, 200 * 1000 + 10); //200msec
		usleep_range(70 * 1000, 70 * 1000 + 10); //70msec

//		rc = lgit_s5kgw1_onsemi_ois_poll_ready(LIMIT_STATUS_POLLING);
		if (rc < 0) {
			CAM_ERR(CAM_OIS, "i2c poll Fail");
		}

		ois_selftest();
		lgit_s5kgw1_onsemi_ois_calibration();
		ois_selftest2();
	}
...
uint32_t lgit_s5kgw1_onsemi_ois_calibration(void)
{
	uint32_t	UlRsltSts;
	...
	RamRead32A(0x338, &val_gyro_offset_x);
	RamRead32A(0x33C, &val_gyro_offset_y);
...
int32_t lgit_s5kgw1_onsemi_ois_stat(sensor_ois_stat_t *data)
{
	...
	/* Gyro Read by reg */
	RamRead32A(0x310, &val_gyro_x);
	RamRead32A(0x314, &val_gyro_y);
	RamRead32A(0x338, &val_gyro_offset_x);
	RamRead32A(0x33C, &val_gyro_offset_y);
...
int32_t lgit_s5kgw1_onsemi_ois_move_lens(void *data, int32_t hall_limit)
{
	int32_t hallx = 0;
	int32_t hally = 0;
	int32_t rHallx = 0;
	...
	RamWrite32A(0x1C4, hallx);
	RamWrite32A(0x218, hally);
	usleep_range(100000, 100010);

	RamRead32A(0x1C8, (uint32_t *)&rHallx);
	RamRead32A(0x21C, (uint32_t *)&rHally);
	...
	CAM_ERR(CAM_OIS, "move fail rHallx: %d, rHally: %d, hall_limit:%d ", abs((rHallx >> 16) - offset[0]), abs((rHally >> 16) - offset[1]), hall_limit);
	return OIS_FAIL;
}
...
static int8_t ois_selftest(void)
{
	...
	int32_t hall_limit = 0;

	int16_t hall_target_0[9][2] = { //FW Ver : 0x04~0x06
	{4242, 0}, {4242, 4014}, {0, 4014},
	{-4242, 4014}, {-4242, 0}, {-4242, -4014},
	{0, -4014}, {4242, -4014}, {4242, 0}
	};

	int16_t hall_target_1[9][2] = { //FW Ver : 0x08~
	{5910, 0}, {4179, 4053}, {0, 5733},
	{-4179, 4053}, {-5910, 0}, {-4179, -4053},
	{0, -5733}, {4179, -4053}, {4179, -4053}
	};

	ois_i2c_e2p_read(MODULE_FW_VER_ADDR, &m_fw_ver, CAMERA_SENSOR_I2C_TYPE_BYTE);
	memset(hall_target, 0, sizeof(hall_target_0));
	if(m_fw_ver < 0x08){
		memcpy(hall_target,hall_target_0,sizeof(hall_target_0));
		hall_limit = 1082;
	} else {
		memcpy(hall_target,hall_target_1,sizeof(hall_target_1));
		hall_limit = 1176;
	}
	...
	lgit_s5kgw1_onsemi_ois_stat(&ois_stat);
	...
	for (i = 0; i <= 8; i++) {
		if (lgit_s5kgw1_onsemi_ois_move_lens(&hall_target[i],hall_limit) >= 0)
			result |= (1 << (8 - i));
	}
	...
	//5. reset lens position before ois turn-on.
	{
		...
		lgit_s5kgw1_onsemi_ois_move_lens(&offset,hall_limit);
	}
...
static int8_t ois_selftest2(void)
{
	...
	for (i = 0; i < 100; i++) {
		rc = lgit_s5kgw1_onsemi_ois_stat(&ois_stat);
		if (rc < 0) {
