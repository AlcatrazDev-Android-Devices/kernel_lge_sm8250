...
static enum tfa98xx_error tfa_cont_write_vstepMax2_One
...
static enum tfa98xx_error tfa_cont_write_vstepMax2
...
(struct tfa_device *tfa, int size, uint8_t data[])
{
	enum tfa98xx_error err = TFA98XX_ERROR_OK;
	uint8_t *msg = NULL;
	...
	struct tfa_file_dsc *file, int vstep_idx, int vstep_msg_idx)
{
	...
	if ((type == msg_hdr)
		...
		sscanf(sub_ver_string, "%d", &subversion);
		...
		if ((subversion > 0)
			...
			if (tfa->is_probus_device) {
				 ...
				 * (example: For climax --calibrate scenario
				 * for probus devices)
				 */
				...
				for (i = 0; i < 3; i++) {
					if (tfa->fw_itf_ver[i] != hdr->customer[i + 4]) {
						...
						pr_err("%s: Error: Expected FW API version = %d.%d.%d, Msg File version: %d.%d.%d\n",
							__func__,
							tfa->fw_itf_ver[0],
							tfa->fw_itf_ver[1],
							tfa->fw_itf_ver[2],
	...
	case volstep_hdr:
		if (tfa->tfa_family == 2)
			err = tfa_cont_write_vstepMax2(tfa,
 ...
 */
enum tfa98xx_error tfa_cont_get_slave(struct tfa_device *tfa,
	uint8_t *slave_addr)
{
...
void tfa_cont_set_slave(uint8_t slave_addr)
{
...
(struct tfa_device *tfa, struct tfa_desc_ptr *dsc)
{
	...
	case dsc_register: /* register patch */
		reg = (struct tfa_reg_patch *)(dsc->offset + (uint8_t *)tfa->cnt);
	...
	case dsc_patch:
		break;
	case dsc_mode:
		cas = (struct tfa_mode *)(dsc->offset + (uint8_t *)tfa->cnt);
	...
	case dsc_cf_mem:
		...
		break;
	case dsc_bit_field:
		bitf = (struct tfa_bitfield *)(dsc->offset + (uint8_t *)tfa->cnt);
...
unsigned int tfa98xx_get_profile_chsa(struct tfa_device *tfa)
{
	struct tfa_bitfield *bitf;
	unsigned int i;
	struct tfa_device_list *dev;
	...
	pr_debug("%s - profile chsa: 0x%x (%d - %d)\n", __func__,
		 chsa_profile, tfa->dev_idx, tfa->profile);
	if (chsa_profile != -1)
		return chsa_profile;
	...
	chsa_profile = tfa_get_bf(tfa, TFA1_BF_CHSA);
...
unsigned int tfa98xx_get_profile_tdmspks(struct tfa_device *tfa)
{
	struct tfa_bitfield *bitf;
	unsigned int i;
	struct tfa_device_list *dev;
	...
	for (i = 0; i < prof->length; i++) {
		...
		if (prof->list[i].type == dsc_bit_field) {
			...
			case 0x72:
				...
				break;
			case 0x78:
				if (bitf->field == TFA9878_BF_TDMSPKS)
					tdmspks_profile = bitf->value;
				break;
			default:
				break;
	...
	pr_debug("%s - profile tdmspks: 0x%x (%d - %d)\n", __func__,
		 tdmspks_profile, tfa->dev_idx, tfa->profile);
	if (tdmspks_profile != -1)
	...
	switch (tfa->rev & 0xff) {
	case 0x72:
		tdmspks_profile = tfa_get_bf(tfa, TFA9872_BF_TDMSPKS);
		break;
	case 0x78:
		tdmspks_profile = tfa_get_bf(tfa, TFA9878_BF_TDMSPKS);
		break;
	default:
	...
	int fs_previous_profile)
{
	...
	if (fs_next_profile != fs_default_profile) {
		 ...
		 * AUDFS from the default profile is not the same as AUDFS
	...
	int prof_idx, int vstep_idx)
{
	...
	/* TODO?: put SetRe25C message to indicate all messages are send */
	if (tfa->ext_dsp) { /* check only master device */
		/* put SetRe25C message to indicate all messages are send */
	...
	int dev_idx, int prof_idx)
{
	...
	prof_name_len = strlen(tfa_cont_profile_name(cnt, dev_idx, prof_idx));
	pch = strnchr(tfa_cont_profile_name(cnt, dev_idx, prof_idx),
		strlen(tfa_cont_profile_name(cnt, dev_idx, prof_idx)), '.');
	if (!pch)
		return 0;
	...
	/* Check if next profile is tap profile */
	if (strnstr(tfa_cont_profile_name(cnt, dev_idx, prof_idx),
	...
	int length, const char *buffer)
{
	...
	// 	? MAX_APR_MSG_SIZE : (MAX_APR_MSG_SIZE * 3 / 4); /* 5336 : 4000; */
	/* TEMPORARY, 16KB to disable concatenation, but to utilize dsp_msg_packet */
	...
	if (length == -1) {
		...
		if (tfa->convert_dsp32) {
			if (total <= 0xffff) {
				blob[2] = (uint8_t)((total >> 8) & 0xff);
				blob[3] = (uint8_t)(total & 0xff);
			}
		} else {
			if (total <= 0xff)
				blob[0] = (uint8_t)(total & 0xff);
		}
#endif
		...
		if (blob_p_index != -1) {
			tfa98xx_buffer_pool_access
				(blob_p_index, 0, &blob, POOL_RETURN);
			blob_p_index = -1;
		} else {
			kfree(blob);
		}
...
#if defined(TFADSP_DSP_BUFFER_POOL)
		if (blob_p_index != -1) {
			pr_err("%s: another request when buffer is already allocated\n",
				__func__);
			tfa98xx_buffer_pool_access
				(blob_p_index, 0, &blob, POOL_RETURN);
		}
