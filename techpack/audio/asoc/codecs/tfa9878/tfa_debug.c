...
uint16_t tfa_cont_bf_enum(const char *name, unsigned short rev)
{
	...
	case 0x74:
		...
		if (bfnum == 0xffff)
			bfnum = tfa_name2bf(tfa9874_bit_names, name);/* try long bitname table */
		break;
	case 0x78:
		bfnum = tfa_name2bf(tfa9878_datasheet_names, name);
		if (bfnum == 0xffff)
			bfnum = tfa_name2bf(tfa9878_bit_names, name);/* try long bitname table */
		break;
	case 0x92:
	...
	case 0x94:
		...
		if (((rev >> 8) & 0xff) == 0x2a || ((rev >> 8) & 0xff) == 0x3a)
		{
			...
			if (bfnum == 0xffff)
				bfnum = tfa_name2bf(tfa9894N2_bit_names, name);/* try long bitname table */
		}
		else
		{
			bfnum = tfa_name2bf(tfa9894_datasheet_names, name);
			if (bfnum == 0xffff)
				bfnum = tfa_name2bf(tfa9894_bit_names, name);/* try long bitname table */
		}
#endif
		bfnum = tfa_name2bf(tfa9894_datasheet_names, name);
		if (bfnum == 0xffff)
