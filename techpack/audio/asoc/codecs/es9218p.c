...
static int __es9218_sabre_headphone_on(void)
{
    ...
    if (es9218_power_state == ESS_PS_CLOSE) {
        ...
        // guarantee enough time for plug re-check run like 250ms
        schedule_delayed_work(&g_es9218_priv->plug_headphone_on_work, msecs_to_jiffies(300));
        return 0;
...
static int __es9218_sabre_headphone_off(void)
{
    ...
    cancel_delayed_work_sync(&g_es9218_priv->plug_headphone_on_work);
...
static void es9218_sabre_plug_headphone_on_work(struct work_struct *work)
{
    // delayed work called by the wbhc plug in
    es9218p_sabre_hifi2lpb();
    es9218p_sabre_bypass2hifi();
    es9218_call_avc_check();
    pr_info("%s(Alc) work headset:%d\n", __func__, g_headset_type);
    return;
}
...
static int es9218_probe(struct i2c_client *client,const struct i2c_device_id *id)
{
    ...
    INIT_DELAYED_WORK(&g_es9218_priv->plug_headphone_on_work, es9218_sabre_plug_headphone_on_work);
