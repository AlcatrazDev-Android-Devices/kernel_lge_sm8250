...
#define AW882XX_VERSION "v0.1.19"
 ...
 *
 ******************************************************/
#define AW882XX_ATTR_CALI_COUNT	3
...
{
	aw_dev_err(aw882xx->dev, "%s: enter pwd:%d\n", __func__,pwd);
...
static bool aw882xx_get_pwd(struct aw882xx *aw882xx)
{
	unsigned int ret = 0;
	aw_dev_dbg(aw882xx->dev, "%s: enter\n", __func__);

	aw882xx_i2c_read(aw882xx, AW882XX_SYSCTRL_REG, &ret);
	aw_dev_dbg(aw882xx->dev, "%s: SYSCTRL 0x%x\n", __func__, ret);
	if (ret & AW882XX_PWDN_POWER_DOWN_VALUE) {
		return true;
	} else {
		return false;
	}
...
static void aw882xx_start(struct aw882xx *aw882xx)
{
	...
	if (ret < 0) {
		...
		aw882xx->is_power_on = AW882XX_CLOSE_PA;
	} else {
		...
		aw882xx->is_power_on = AW882XX_OPEN_PA;
	}
...
static void aw882xx_reg_loaded(const struct firmware *cont, void *context)
{
	...
	aw_dev_dbg(aw882xx->dev, "%s: lock enter\n", __func__);
	mutex_lock(&aw882xx->lock);
	...
	mutex_unlock(&aw882xx->lock);
	aw_dev_dbg(aw882xx->dev, "%s: lock exit\n", __func__);
	kfree(aw882xx_cfg);
...
void aw882xx_smartpa_cfg(struct aw882xx *aw882xx, bool flag)
{
	...
	if (flag == true && aw882xx->scene_mode != AW882XX_OFF_MODE) {
 ...
 *
 ******************************************************/
static const char *const mode_function[] = { "Spk", "Rcv", "Off"};
	...
	struct snd_ctl_elem_value *ucontrol)
{
	...
	if (aw882xx->scene_mode != AW882XX_OFF_MODE)
	...
	struct snd_ctl_elem_value *ucontrol)
{
	...
	if (aw882xx_get_pwd(aw882xx)) {
		aw_dev_info(aw882xx->dev, "%s: pwdn state\n", __func__);
		ucontrol->value.integer.value[0] = 0;
		return 0;
	}

	ret = aw_read_data_from_dsp(INDEX_PARAMS_ID_RX_ENBALE,
				&ctrl_value, sizeof(uint32_t),
				aw882xx->chan_info.channel);
	if (ret)
	...
	struct snd_ctl_elem_value *ucontrol)
{
	...
	ret = aw_send_afe_module_enable(&ctrl_value,
			sizeof(uint32_t), AW_RX_MODULE);
	if (ret)
		aw_dev_err(aw882xx->dev, "%s: dsp_msg error, ret=%d\n",
			__func__, ret);
	...
	struct snd_ctl_elem_value *ucontrol)
{
	...
	ret = aw_read_data_from_dsp(INDEX_PARAMS_ID_TX_ENABLE,
				&ctrl_value, sizeof(uint32_t),
				chan_info->channel);
	if (ret)
		aw_dev_err(aw882xx->dev, "%s: dsp_msg error, ret=%d\n",
		...
		struct snd_ctl_elem_value *ucontrol)
{
	...
	if (ucontrol->value.integer.value[0] == *pa_switch_ptr)
		return 1;

	*pa_switch_ptr = ucontrol->value.integer.value[0];

	if (*pa_switch_ptr == AW882XX_CLOSE_PA &&
			aw882xx->is_power_on == AW882XX_OPEN_PA)
		aw882xx_smartpa_cfg(aw882xx, false);
	else if (*pa_switch_ptr == AW882XX_OPEN_PA &&
			aw882xx->is_power_on == AW882XX_CLOSE_PA)
...
{
	int i = 0;
	struct snd_kcontrol_new *dst_control;
...
static int aw882xx_mute(struct snd_soc_dai *dai, int mute, int stream)
{
	...
	if (mute) {
		...
		if (stream == SNDRV_PCM_STREAM_PLAYBACK &&
				aw882xx->aw882xx_pa_switch == AW882XX_OPEN_PA)
			aw882xx_smartpa_cfg(aw882xx, true);
	}
...
{
	if(NULL == aw882xx  || NULL == aw882xx->dev)
	{
		printk("%s audiock aw882xx is null .\n",__func__);
		return 0;
	}
	else
	{
		//aw_dev_info(aw882xx->dev, "%s: enter\n", __func__);
		printk("%s audiock aw882xx enter .\n",__func__);
		if (aw882xx && gpio_is_valid(aw882xx->reset_gpio)) {
			...
			printk("%s audiock aw882xx is failed .\n",__func__);
		}
	}
	return 0;
	...
	const struct i2c_device_id *id)
{
	...
	aw882xx_append_suffix("%s-%s", &dai->name, aw882xx);
	aw882xx_append_suffix("%s_%s", &dai->playback.stream_name, aw882xx);
	aw882xx_append_suffix("%s_%s", &dai->capture.stream_name, aw882xx);
	...
	aw882xx->aw882xx_pa_switch = AW882XX_OPEN_PA; /*can open*/
	aw882xx->is_power_on = AW882XX_CLOSE_PA;
