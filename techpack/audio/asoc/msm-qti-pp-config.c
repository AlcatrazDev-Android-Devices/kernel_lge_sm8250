...
static int msm_afe_sen_mi2s_lb_vol_ctrl;
...
static int msm_qti_pp_get_sen_mi2s_lb_vol_mixer(struct snd_kcontrol *kcontrol,
				       struct snd_ctl_elem_value *ucontrol)
{
	ucontrol->value.integer.value[0] = msm_afe_sen_mi2s_lb_vol_ctrl;
	return 0;
}

static int msm_qti_pp_set_sen_mi2s_lb_vol_mixer(struct snd_kcontrol *kcontrol,
			    struct snd_ctl_elem_value *ucontrol)
{
	afe_loopback_gain(AFE_PORT_ID_SENARY_MI2S_TX,
			  ucontrol->value.integer.value[0]);
	msm_afe_sen_mi2s_lb_vol_ctrl = ucontrol->value.integer.value[0];
	return 0;
}
...
static const struct snd_kcontrol_new sen_mi2s_lb_vol_mixer_controls[] = {
	SOC_SINGLE_EXT_TLV("Sen MI2S LOOPBACK Volume", SND_SOC_NOPM, 0,
	INT_RX_VOL_GAIN, 0, msm_qti_pp_get_sen_mi2s_lb_vol_mixer,
	msm_qti_pp_set_sen_mi2s_lb_vol_mixer, afe_lb_vol_gain),
};
...
void msm_qti_pp_add_controls(struct snd_soc_component *component)
{
	...
	snd_soc_add_component_controls(component,
			sen_mi2s_lb_vol_mixer_controls,
			ARRAY_SIZE(sen_mi2s_lb_vol_mixer_controls));
