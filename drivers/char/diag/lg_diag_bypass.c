...
static char *bypass_request = NULL;
static char *bypass_response = NULL;
...
int diag_bypass_response(unsigned char* buf, int len, int proc, int ctxt, struct diag_logger_t *logger)
{
    if(diag_bypass_enable) {
        ...
        if(ret > 0) {
            ...
            diagfwd_mux_write_done_bypass(buf, len, ctxt, proc);
            return 1;
        }
        return 0;
    }
...
static int __init lge_bypass_init(void)
{
    ...
    pr_info("%s\n", __func__);
    bypass_request = kzalloc(0x1000, GFP_KERNEL);
    if (!bypass_request) {
        pr_err( "%s: bypass_request memory alloc fail\n", __func__);
        return 0;
    }

    bypass_response = kzalloc(0x1000, GFP_KERNEL);
    if (!bypass_response) {
        pr_err( "%s: bypass_response memory alloc fail\n", __func__);
        return 0;
    }
...
static void __exit lge_bypass_exit(void)
{
    ...
    if (!bypass_request) {
        pr_err("%s: bypass_request is null\n", __func__);
    }
    else {
        kfree(bypass_request);
    }

    if (!bypass_response) {
        pr_err("%s: bypass_response is null\n", __func__);
    }
    else {
        kfree(bypass_response);
    }
