...
#ifdef CONFIG_CXD224X_NFC_SERVICE_MODE
...
#ifdef CONFIG_CXD224X_NFC_SERVICE_MODE
     cxd224x_dev->tst1_gpio = of_get_named_gpio_flags(np, "sony,gpio_tst1", 0, NULL);
#endif
...
#ifdef CONFIG_CXD224X_NFC_SERVICE_MODE
    ret = gpio_request_one(cxd224x_dev->tst1_gpio, GPIOF_OUT_INIT_LOW, "nfc_tst1");
    if (ret)
        goto err_exit;
    tst1_gpio_ok=1;
    gpio_set_value(cxd224x_dev->tst1_gpio, 0);
