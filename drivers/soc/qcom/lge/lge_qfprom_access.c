...
static u32 qfprom_check_serialbind_status(void);
...
static ssize_t qfusing_show(struct device *dev, struct device_attribute *attr, char *buf)
{
  ...
  qfprom_check_serialbind_status();
...
static ssize_t qfprom_deviceid_show(struct device *dev, struct device_attribute *attr, char *buf)
{
  u32 lsb = 0, msb = 0, i = 0;
  void *temp = NULL;
  unsigned char device_id_hash[SHA256_SIZE] = {0,};
  unsigned char device_id_char[SHA256_SIZE_CHAR+1] = {0,};
  ...
  ahash_request_set_crypt(req, &sg, device_id_hash, DEVICE_ID_INPUT_SIZE);
...
static u32 qfprom_check_serialbind_status(void)
{
  ...
  u32 ret = 0;
  u32 serialnum_status = SERIAL_NUM_WITHOUT;

  desc.args[0] = 0;
  desc.arginfo = 0;
  ret = scm_call2(SCM_SIP_FNID(SCM_SVC_UTIL, TZ_DUMP_SECURITY_ALLOWS_MEM_DUMP), &desc);
  if (ret) {
    printk(KERN_ERR "[QFUSE]%s : SCM call failed!!\n", __func__);
    return SERIAL_NUM_CHECK_FAIL;
  }
  printk(KERN_INFO "[QFUSE]%s : serial_binding desc.ret = 0x%02X\n", __func__, desc.ret[2]);

  /* desc.ret[2] value
   * 0   bit of desc.ret[2] : DP_ENABLE_ONLINE_CRASH_DUMPS flag
   * 1st bit of desc.ret[2] : DP_ENABLE_OFFLINE_CRASH_DUMPS flag
   * 2nd bit of desc.ret[2] : DP_ENABLE_JTAG flag
   * 3rd bit of desc.ret[2] : DP_ENABLE_LOGS flag
   * 4th bit of desc.ret[2] : DP_ENABLE_NONSECURE_CRASH_DUMPS flag
   */
  if ((desc.ret[2] & DP_ONLINE_CRASH_DUMPS)) {
    serialnum_status = SERIAL_NUM_MATCHED; //with matched serial_num
    printk(KERN_INFO "[QFUSE]%s : serial_binding status = %d : SERIAL_NUM_MATCHED\n", __func__, ret);
  } else {
    if ((desc.ret[2] & DP_NONSECURE_CRASH_DUMPS)) {
      serialnum_status = SERIAL_NUM_WITHOUT; //without serial_num
      printk(KERN_INFO "[QFUSE]%s : serial_binding status = %d : SERIAL_NUM_WITHOUT\n", __func__, ret);
    } else {
      serialnum_status = SERIAL_NUM_NOT_MATCHED; //with not-matched serial_num
      printk(KERN_INFO "[QFUSE]%s : serial_binding status = %d : SERIAL_NUM_NOT_MATCHED\n", __func__, ret);
    }
  }
  return serialnum_status;
}

static ssize_t qfprom_serialbind_show(struct device *dev, struct device_attribute *attr, char *buf)
{
  u32 ret = SERIAL_NUM_WITHOUT;

  printk(KERN_INFO "[QFUSE]%s start\n", __func__);
  ret = qfprom_check_serialbind_status();
  printk(KERN_INFO "[QFUSE]%s end\n", __func__);
  switch (ret)
  {
    case SERIAL_NUM_MATCHED:
      return sprintf(buf, "%s\n", "SN matched");
    case SERIAL_NUM_NOT_MATCHED:
      return sprintf(buf, "%s\n", "SN not-matched");
    case SERIAL_NUM_WITHOUT:
      return sprintf(buf, "%s\n", "W/O SN");
    case SERIAL_NUM_CHECK_FAIL:
      return sprintf(buf, "%s\n", "Error!!");
    default:
      return sprintf(buf, "%s\n", "Not supported!!");
  }
}
static DEVICE_ATTR(serial_bnd, S_IWUSR | S_IRUGO, qfprom_serialbind_show, NULL);
...
static u32 qfprom_secdat_read(void)
{
  ...
  temp1 = kmalloc(sizeof(fuseprov_secdat_hdr_type), GFP_KERNEL);
  if (temp1 == NULL)
    goto err_mem;
  memcpy(temp1, (char*)&secdat.hdr, sizeof(fuseprov_secdat_hdr_type));
  sg_set_buf(&sg[sg_idx++], temp1, sizeof(fuseprov_secdat_hdr_type));
  ...
  if(secdat.hdr.revision >= 2 && secdat.hdr.segment_number !=0)
  {
    for(i=0; i < secdat.hdr.segment_number ; i++)
    {
      ...
      temp2 = kmalloc(sizeof(fuseprov_secdat_hdr_segment_type), GFP_KERNEL);
      if (temp2 == NULL)
        goto err_mem;
      memcpy(temp2, (char*)&secdat.segment, sizeof(fuseprov_secdat_hdr_segment_type));
      sg_set_buf(&sg[sg_idx++], temp2, sizeof(fuseprov_secdat_hdr_segment_type));
    }
  }
  ...
  temp3 = kmalloc(sizeof(fuseprov_qfuse_list_hdr_type), GFP_KERNEL);
  if (temp3 == NULL)
    goto err_mem;
  memcpy(temp3, (char*)&secdat.list_hdr, sizeof(fuseprov_qfuse_list_hdr_type));
  sg_set_buf(&sg[sg_idx++], temp3, sizeof(fuseprov_qfuse_list_hdr_type));
  ...
  temp4 = kmalloc(sizeof(fuseprov_secdat_footer_type), GFP_KERNEL);
  if (temp4 == NULL)
    goto err_mem;
  memcpy(temp4, (char*)&secdat.footer, sizeof(fuseprov_secdat_footer_type));
  sg_set_buf(&sg[sg_idx], temp4, sizeof(fuseprov_secdat_footer_type));
