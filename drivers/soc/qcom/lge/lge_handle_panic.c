...
u64 sctlr_el1, tcr_el1, ttbr0_el1, ttbr1_el1, esr_el1;
u64 far_el1, contextidr_el1, tpidr_el0, tpidr_el1, tpidrro_el0;

static void lge_get_mmu_sys_ctrl_register(void)
{
	asm volatile ("mrs %0, sctlr_el1\n\t"
					...
					"=r"(esr_el1), "=r"(far_el1),
					"=r"(contextidr_el1), "=r"(tpidr_el0),
					"=r"(tpidr_el1), "=r"(tpidrro_el0)
					: : "memory");

	printk("[+]ttbr0_el1: %016llx  ttbr1_el1: %016llx tcr_el1: %016llx sctlr_el1: %016llx esr_el1: %016llx \n",
						ttbr0_el1, ttbr1_el1, tcr_el1, sctlr_el1, esr_el1);
...
static int lge_handler_panic(struct notifier_block *this, unsigned long event, void *ptr)
{
	unsigned long flags;

	spin_lock_irqsave(&lge_panic_lock, flags);

	printk(KERN_CRIT "%s called from: %pS\n", __func__, (void *)__builtin_return_address(0));
	lge_get_mmu_sys_ctrl_register();
