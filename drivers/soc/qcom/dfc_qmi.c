			       ...
			       bool is_query)
{
...
#ifdef CONFIG_LGE_LOG_FOR_UL_STALL
    ...
    //lg_add_to_log_for_ul_stall
#endif /* CONFIG_LGE_LOG_FOR_UL_STALL */
			 ...
			 int ip_type, u32 mark, unsigned int len)
{
    ...
    if (bearer->grant_size <= 10 ) {
		pr_err("[DFC]dfc_flow_check dev=%s, bearer_id=%d, skb_len=%d, current_grant=%d, mk=%d"
			,dev->name,bearer->bearer_id,len,bearer->grant_size, mark);
	}
    //lg_add_to_log_for_ul_stall
#endif /* CONFIG_LGE_LOG_FOR_UL_STALL */
