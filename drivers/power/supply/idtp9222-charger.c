...
static inline bool idtp9222_reg_check(struct idtp9222_struct* idtp9222) {
	u8 address [] = {
		REG_ADDR_FIRMWARE >> 8,
	...
	if (!idtp9222->status_overheat
		...
		&& i2c_transfer(idtp9222->wlc_client->adapter, message, 2) == 2)
		success = true;
...
static bool psy_set_capacity_raw(struct idtp9222_struct* idtp9222, int capacity_raw) {
	...
	if (idtp9222->capacity_raw >= idtp9222->configure_rawfull) {
...
static void idtp9222_polling_volt(struct work_struct* work) {
	...
	if (idtp9222_reg_check(idtp9222)) {
