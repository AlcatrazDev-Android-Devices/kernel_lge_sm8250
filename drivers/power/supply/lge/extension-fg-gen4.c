...
/* v/i ADCs  */ "iBAT:%d, vBAT:%d, vPRD:%d, vOCV:%d, vUSB:%d, iUSB:%d, iPM:%d, iCP:%d, vWLC:%d, iWCL:%d, AiCL:%d, "
...
/* Input     */ fginfo.input_vusb, fginfo.input_iusb, fginfo.input_ipm, fginfo.input_icp, fginfo.input_vwlc, fginfo.input_iwlc, fginfo.input_aicl,
...
static void
fginfo_snapshot_inputnow(int* vusb, int* iusb, int* vwlc, int* iwlc, int* aicl, int *ipm, int* icp)
{
	union power_supply_propval val = { 0, };
	...
	struct power_supply* psy_cp_master = power_supply_get_by_name("charge_pump_master");
	...
	*iusb = *ipm + *icp;
	...
	if (psy_cp_master)
		power_supply_put(psy_cp_master);
}
...
/* Input */
	fginfo_snapshot_inputnow(&fginfo.input_vusb, &fginfo.input_iusb,
		&fginfo.input_vwlc, &fginfo.input_iwlc,	&fginfo.input_aicl,
		&fginfo.input_ipm, &fginfo.input_icp);
/* Temperature */
...
static int extension_fg_gen4_adjust_ki_coeff_cutoff(
	struct fg_gen4_chip *chip, int batt_temp)
{
	struct fg_dev *fg = &chip->fg;
	int rc = 0;
	static int ki_coeff_cutoff_gain = -EINVAL;
	u8 val;

	if (!chip->ki_coeff_cutoff_en)
		return 0;

	if (batt_temp < 0 && fg->charge_status == POWER_SUPPLY_STATUS_DISCHARGING) {
		if (ki_coeff_cutoff_gain == chip->dt.ki_coeff_cutoff_lt)
			return 0;
		ki_coeff_cutoff_gain = chip->dt.ki_coeff_cutoff_lt;
	} else {
		if (ki_coeff_cutoff_gain == chip->dt.ki_coeff_cutoff_gain)
			return 0;
		ki_coeff_cutoff_gain = chip->dt.ki_coeff_cutoff_gain;
	}

	fg_encode(fg->sp, FG_SRAM_KI_COEFF_CUTOFF, ki_coeff_cutoff_gain, &val);
	rc = fg_sram_write(fg,
	...
	fg_dbg(fg, FG_LGE, "Wrote ki_coeff_cutoff_gain = %d(0x%X), batt_temp = %d\n",
		ki_coeff_cutoff_gain, val, batt_temp);

	return 0;
}

static int extension_fg_parse_ki_coefficients(struct fg_dev *fg, struct device_node *node)
{
	u8 val;
	int rc = -1;
	struct fg_gen4_chip *chip = container_of(fg, struct fg_gen4_chip, fg);
	...
	chip->dt.ki_coeff_cutoff_lt = -EINVAL;
	of_property_read_u32(node, "qcom,ki-coeff-cutoff-lt",
		&chip->dt.ki_coeff_cutoff_lt);

	chip->ki_coeff_cutoff_en = true;
	if (chip->dt.ki_coeff_cutoff_gain == -EINVAL
		|| chip->dt.ki_coeff_cutoff_lt == -EINVAL)
	chip->ki_coeff_cutoff_en = false;
	...
	pr_info("FG: battery ki-coeff-chg: %d, %d, %d, "
			"cutoff ki-coeff: en=%d, %d, %d\n",
		chip->dt.ki_coeff_low_chg,
		chip->dt.ki_coeff_med_chg,
		chip->dt.ki_coeff_hi_chg,
		chip->ki_coeff_cutoff_en,
		chip->dt.ki_coeff_cutoff_gain,
		chip->dt.ki_coeff_cutoff_lt);
	return 0;
}
	...
	struct fg_dev *fg, struct device_node* container, int resistance_id)
{
	...
	extension_fg_parse_ki_coefficients(fg, profile);
...
static int extension_fg_load_icoeff_dt(struct fg_dev *fg)
{
	...
	if (fg->bp.batt_type_str) {
		tcomp_override = of_find_node_by_name(
				tcomp_dtroot, fg->bp.batt_type_str);
		if (tcomp_override &&
