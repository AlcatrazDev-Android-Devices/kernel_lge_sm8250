...
static bool is_fcc_voter_available(void)
{
	if (!ext_qnovo.fcc_votable) {
		ext_qnovo.fcc_votable = find_votable("FCC");
		if (!ext_qnovo.fcc_votable) {
			return false;
		}
	}
...
static void input_ready_work(struct work_struct *work)
{
	...
	pr_info("cnt=%d, fcc=%d, now=%d, best=%d, worst=%d\n",
		ext_qnovo.pt_manager.cnt,
		now_fcc/1000,
		now_curr/1000,
...
static void pt_start_watchdog_work(struct work_struct *work)
{
	...
	if (pt_time <= 0) {
		...
		pr_info("pt start bite!! pt_t=%d, cnt=%d\n",
...
{
	if (!(get_qps() == QPS_PRE_DEFINED_USB || get_qps() == QPS_IN_RUNTIME)) {
...
static int monitor_fcc(struct qnovo *chip)
{
	...
	int rc = -1;
	int now_fcc = 0, now_curr = 0,  now_health = 0;
	bool is_diag_stage = false;
	...
	if (get_qps() == QPS_IN_RUNTIME) {
		...
		if (now_fcc == 500000 && is_fcc_voter_available() &&
			(!strncmp(get_effective_client(ext_qnovo.fcc_votable),
						"QNOVO_VOTER", 11)))
			is_diag_stage = true;
	}
	...
	pr_info("[QNI-PROB] run-time monitor_fcc: "
			"curr=%d, now=%d, old=%d, locked=%d, health: %s\n",
...
static int rt_qni_probation(struct qnovo *chip)
{
	...
	pr_info(
		"[QNI-PROB] set run-time qni probation: "
...
static bool pred_qni_probation(struct qnovo *chip)
{
	...
	is_pred_probation = pred_usb_probation(chip);
...
static int override_qnovo5_update_status(struct qnovo *chip)
{
	...
	if (status_raw.intval != POWER_SUPPLY_STATUS_CHARGING) {
		...
		if (get_qps() < QPS_PRE_DEFINED_USB)
			goto out;
	...
	if ((chip->usb_present != usb_present) || (chip->charging != charging)) {
		if (usb_present && charging) {
			if (!chip->usb_charging && msoc < 255) {
				set_usb_debounce(chip, 1);
				pr_info("usb=1, charging=1...wait 15sec\n");
			}
		}
		else {
			if (chip->usb_charging) {
				...
				qnovo_fet_ctrl(chip);
				pr_info("usb=%d, charging=%d...disabled\n",
...
static irqreturn_t override_handle_ptrain_done(int irq, void *data)
{
	...
	if (!done_pt_manager(chip)) {
		pr_info("diage stage is over-counted(>20). qnovo restart!!\n");
		if (chip->usb_charging) {
			set_usb_debounce(chip, 0);
			set_usb_debounce(chip, 1);
		}
...
static int extension_qnovo_parse_dt(struct qnovo *chip)
{
	...
	pr_info("[QNI-DTSI] probation=%d, fcc_compensate=%d, qna_unsafety_protection=%d, "
