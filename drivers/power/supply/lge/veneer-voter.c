...
bool veneer_voter_changed(enum voter_type type)
{
	...
	/* clear is_changed flag after reading */
	mutex_lock(&list->lock);
	if (list->is_changed == true) {
		changed = true;
...
void veneer_voter_set(struct voter_entry* voter, int limit) {
	...
	if (list_of) {
		...
		if (strncmp(voter->name, "CP_PPS", 6) && (voter->type == VOTER_TYPE_IBAT)) {
			list_of->is_changed = true;
			pr_voter("%s ibat voter is changed: %d -> %d\n",
				voter->name, voter->limit, limit);
		}
		voter->limit = limit;
