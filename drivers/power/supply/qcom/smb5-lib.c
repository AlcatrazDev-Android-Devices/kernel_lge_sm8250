...
#define LPD_RECHECK_TIME	60000
#define LPD_RESUME_TIME		2000

static int smblib_set_moisture_detected(struct smb_charger *chg, bool moisture);
#endif
...
int smblib_vconn_regulator_enable(struct regulator_dev *rdev)
{
	 ...
	 */
	if (chg->ext_chg->is_usb_compliance_mode)
...
#ifdef CONFIG_LGE_USB
	if (!val->intval &&
	    (stat & SNK_SRC_MODE_BIT) && (stat & TYPEC_VBUS_STATUS_BIT)) {
		rc = smblib_read(chg, TYPE_C_CC_STATUS_REG, &stat);
		if (rc < 0) {
			smblib_err(chg, "Couldn't read TYPE_C_CC_STATUS_REG rc=%d\n",
				   rc);
			return rc;
		}
		smblib_dbg(chg, PR_REGISTER, "TYPE_C_CC_STATUS_REG = 0x%02x\n", stat);

		if (stat == 0x08 || stat == 0x80)
			val->intval = (bool)(stat == 0x08) + 1;
	}
#endif
...
static int smblib_get_prop_ufp_mode(struct smb_charger *chg)
{
	...
	if (rc < 0) {
#ifdef CONFIG_LGE_USB
		smblib_err(chg, "Couldn't read TYPE_C_SNK_STATUS_REG rc=%d\n",
			   rc);
#else
...
#ifdef CONFIG_LGE_USB
	smblib_dbg(chg, PR_REGISTER, "TYPE_C_SNK_STATUS = 0x%02x\n", stat);
#else
...
#ifdef CONFIG_LGE_PM
	smblib_dbg(chg, PR_MISC, "power_role = %d(0x%x)\n", val->intval, power_role);
#endif
				...
				const union power_supply_propval *val)
{
	...
	if (chg->pd_active) {
		...
		if (chg->lpd_reason != LPD_NONE)
			dev_err(chg->dev, "LPD reason MOISTURE_DETECTED or FLOATING\n");
...
#ifdef CONFIG_LGE_USB_MOISTURE_DETECTION
int smblib_set_prop_moisture_detected(struct smb_charger *chg,
				const union power_supply_propval *val)
{
	union power_supply_propval pval = {0, };
	int rc = 0;
	bool lpd_flag = false;
#ifdef CONFIG_LGE_USB_SBU_SWITCH
	unsigned long sbu_switch_flag;
#endif

	lpd_flag = val->intval == POWER_SUPPLY_MOISTURE_DETECTED ? true : false;

	if (lpd_flag) {
		chg->lpd_stage = LPD_STAGE_COMMIT;
		rc = smblib_set_moisture_detected(chg, true);
		if (rc < 0)
			return rc;
...
#ifdef CONFIG_LGE_USB_SBU_SWITCH
		sbu_switch_flag = lge_sbu_switch_get_current_flag(chg->sbu_inst);
		if (sbu_switch_flag == LGE_SBU_SWITCH_FLAG_SBU_DISABLE)
			lge_sbu_switch_put(chg->sbu_inst, LGE_SBU_SWITCH_FLAG_SBU_DISABLE);
#endif
		rc = smblib_set_moisture_detected(chg, false);
		if (rc < 0)
			return rc;
...
//		chg->pd_not_supported = true;
		...
		if (*chg->lpd_ux)
			...
			vote(chg->usb_icl_votable, LPD_VOTER, true, 1000000);
		...
		/* Set hvdcp_disable true for disable boost voltage in QC
		 * This also has the effect of not displaying the fast charging UI.
		 * If you want fast charging UI only set voltage in get_prop_usb_voltage */
...
static bool smblib_src_lpd(struct smb_charger *chg)
{
	...
	if (check_ds_connect_state() > DS_STATE_DISCONNECTED)
	{
		if (!(*chg->lpd_ux) ||
				chg->lpd_reason != LPD_MOISTURE_DETECTED)
			return false;
...
static void typec_src_fault_condition_cfg(struct smb_charger *chg, bool src)
{
	...
	rc = smblib_masked_write(chg, USB_CMD_PULLDOWN_REG,
			...
			src ? 0 : EN_PULLDOWN_USB_IN_BIT);
	if (rc < 0)
		smblib_err(chg, "Couldn't write USB_CMD_PULLDOWN_REG rc=%d\n", rc);
#endif
}
...
#ifdef CONFIG_LGE_USB
	smblib_dbg(chg, PR_REGISTER, "LEGACY_CABLE_STATUS = 0x%02x\n", stat);
#endif
...
increment:
	...
	chg->wls_icl_ua = voting_icl - (voting_icl % chg->param.dc_icl.step_u);
#endif
...
#ifdef CONFIG_LGE_PM
	icl = min(chg->wls_icl_ua, icl + (chg->param.dc_icl.step_u * 2));
#else
...
static void smblib_chg_termination_work(struct work_struct *work)
{
	 ...
	 */
#ifdef CONFIG_LGE_PM
	delay = wa_protect_overcharging(chg, input_present);
#else
