...
struct sw42902_touch_info {
	...
	struct sw42902_touch_data data[10];
	/* debug info */
} __packed;
...
struct sw42902_touch_debug_info {
	u32 info[3];
	u32 type:24;
	u32 length:8;
} __packed;
...
struct sw42902_data {
	...
	struct sw42902_touch_info info;
	struct sw42902_touch_debug_info debug_info;
...
{
	return (struct sw42902_data *)touch_get_device(to_touch_core(dev));
}
...
extern int sw42902_reg_read(struct device *dev, u16 addr, void *data, int size);
extern int sw42902_reg_write(struct device *dev, u16 addr, void *data, int size);
extern int sw42902_ic_info(struct device *dev);
extern int sw42902_te_info(struct device *dev, char *buf);
extern int sw42902_tc_driving(struct device *dev, int mode);
extern int sw42902_irq_abs(struct device *dev);
extern int sw42902_irq_abs_data(struct device *dev);
extern int sw42902_irq_lpwg(struct device *dev);
extern int sw42902_irq_handler(struct device *dev);
extern int sw42902_check_status(struct device *dev);
extern int sw42902_debug_info(struct device *dev);
extern int sw42902_sleep_ctrl(struct device *dev, int new_status);
/*
...
static inline int sw42902_read_value(struct device *dev,
		u16 addr, u32 *value)
{
	return sw42902_reg_read(dev, addr, value, sizeof(*value));
}

static inline int sw42902_write_value(struct device *dev,
		u16 addr, u32 value)
{
	return sw42902_reg_write(dev, addr, &value, sizeof(value));
}
