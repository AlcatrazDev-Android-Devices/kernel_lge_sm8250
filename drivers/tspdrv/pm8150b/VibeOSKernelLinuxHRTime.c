...
#endif
struct semaphore pm8150b_g_hSemaphore;
...
static enum hrtimer_restart VibeOSKernelTimerProc(struct hrtimer *timer)
{
    ...
    if (VibeSemIsLocked(&pm8150b_g_hSemaphore))
    {
        up(&pm8150b_g_hSemaphore);
    }
...
static void VibeOSKernelLinuxStartTimer(void)
{
    ...
    if (!g_bTimerStarted)
    {
        ...
        sema_init(&pm8150b_g_hSemaphore, NUM_EXTRA_BUFFERS);
...
#endif
    {
        ...
        */
        res = down_interruptible(&pm8150b_g_hSemaphore);  /* wait for the semaphore to be freed by the timer */
        if (res != 0)
        {
    ...
    ** interrupt will release the pm8150b_g_hSemaphore while VibeOSKernelProcessData is executing and the player
    ...
    */
    ...
    if (g_bTimerStarted && !VibeSemIsLocked(&pm8150b_g_hSemaphore))
    {
        int res;

        res = down_interruptible(&pm8150b_g_hSemaphore);
...
static void VibeOSKernelLinuxTerminateTimer(void)
{
    ...
    if (VibeSemIsLocked(&pm8150b_g_hSemaphore)) up(&pm8150b_g_hSemaphore);
}
