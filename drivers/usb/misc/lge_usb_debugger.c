...
static int lge_usb_debugger_probe(struct platform_device *pdev)
{
	...
	dbg->uart_edp_sel_gpio = devm_gpiod_get(dev, "lge,uart-sbu-sel", GPIOD_OUT_LOW);
	...
	/* do not disable console to use earjack debugger */
	dbg->always_console_enable = of_property_read_bool(pdev->dev.of_node, "lge,always-console-enable");
	dev_info(dbg->dev, "always_console_enable : %d \n", dbg->always_console_enable);
