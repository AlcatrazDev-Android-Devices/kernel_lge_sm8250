...
{
	int ldo_en, sbu_oe, sbu_sel, uart_oe, uart_sel = 0;
	unsigned i;
	...
	case LGE_SBU_SWITCH_MODE_SBU_DISABLE:
		...
		if(!lge_sbu_switch->sbu_oe_desc) {
			...
			sbu_sel = lge_sbu_switch->sbu_sel;
		}
		uart_oe = 1;
		break;
	...
	if(!lge_sbu_switch->sbu_oe_desc)
	{
		if (sbu_oe == 0) {
			...
			gpiod_direction_output(lge_sbu_switch->sbu_sel_desc, sbu_sel);
		} else {
			...
			gpiod_direction_input(lge_sbu_switch->sbu_sel_desc);
		}
	}

	if(lge_sbu_switch->uart_oe_desc)
	{
		dev_info(lge_sbu_switch->dev, "BSP_USB :%s: gpiod_direction_output  for uart_oe\n", __func__);
		gpiod_direction_output(lge_sbu_switch->uart_oe_desc, uart_oe);
	}

	if (sbu_oe) {
		if (lge_sbu_switch->sbu_oe_desc) {
			if(lge_sbu_switch->sbu_oe != sbu_oe)
				gpiod_set_value(lge_sbu_switch->sbu_oe_desc, sbu_oe);
			if (lge_sbu_switch->sbu_sel_desc && lge_sbu_switch->sbu_sel != sbu_sel)
				gpiod_set_value(lge_sbu_switch->sbu_sel_desc, sbu_sel);
		...
		if (lge_sbu_switch->sbu_sel_desc && lge_sbu_switch->sbu_sel != sbu_sel)
			gpiod_set_value(lge_sbu_switch->sbu_sel_desc, sbu_sel);
		if (lge_sbu_switch->sbu_oe_desc && lge_sbu_switch->sbu_oe != sbu_oe)
			gpiod_set_value(lge_sbu_switch->sbu_oe_desc, sbu_oe);
		if (lge_sbu_switch->uart_sel_desc && lge_sbu_switch->uart_sel != uart_sel)
			gpiod_set_value(lge_sbu_switch->uart_sel_desc, uart_sel);
		/*
		 ...
		 */
		if (sbu_sel &&
		    ((lge_sbu_switch->sbu_oe != sbu_oe) ||
		     (lge_sbu_switch->sbu_sel != sbu_sel))) {
			switch (i) {
			case LGE_SBU_SWITCH_MODE_SBU_FACTORY_ID:
			case LGE_SBU_SWITCH_MODE_SBU_MD:
			case LGE_SBU_SWITCH_MODE_SBU_MD_ING:
	...
	lge_sbu_switch->uart_oe = uart_oe;
	lge_sbu_switch->uart_sel = uart_sel;
	lge_sbu_switch->cur_flag = BIT(i);

	dev_info(lge_sbu_switch->dev, "/BSP_USB : SBU: OE(%d) SEL(%d), UART: OE(%d) SEL(%d), current flag is \"%s\"\n",
		sbu_oe, sbu_sel, uart_oe, uart_sel, flag_to_string(lge_sbu_switch->cur_flag));
...
static int lge_sbu_switch_probe(struct platform_device *pdev)
{
	...
	} // TIME model is not used

	lge_sbu_switch->sbu_oe_desc = devm_gpiod_get(dev, "lge,oe",
				  lge_sbu_switch->sbu_oe ? GPIOD_OUT_HIGH : GPIOD_OUT_LOW);
	if (IS_ERR(lge_sbu_switch->sbu_oe_desc)) {
		dev_err(dev, "Could not get oe gpio_desc\n");
		lge_sbu_switch->sbu_oe_desc = NULL;
		lge_sbu_switch->sbu_oe = 0;
	} // TIME model is not used
	...
	lge_sbu_switch->uart_oe_desc = devm_gpiod_get(dev, "lge,uart-edp-oe",
				  lge_sbu_switch->uart_oe ? GPIOD_OUT_HIGH : GPIOD_OUT_LOW);
	if (IS_ERR(lge_sbu_switch->uart_oe_desc)) {
		dev_err(dev, "Could not get uart oe gpio_desc\n");
		lge_sbu_switch->uart_oe_desc = NULL;
		lge_sbu_switch->uart_oe = 0;
	} // GPIO84

	lge_sbu_switch->sbu_sel_desc = devm_gpiod_get(dev, "lge,sel",
				  lge_sbu_switch->sbu_sel ? GPIOD_OUT_HIGH : GPIOD_OUT_LOW);
	if (IS_ERR(lge_sbu_switch->sbu_sel_desc)) {
		dev_err(dev, "Could not get sel gpio_desc\n");
		return PTR_ERR(lge_sbu_switch->sbu_sel_desc);
	} // GPIO50
