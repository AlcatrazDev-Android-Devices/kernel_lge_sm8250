	...
	struct mtp_file_range *mfr)
{
	...
	if (mtp_lock(&dev->ioctl_excl)) {
		ERROR(dev->cdev, "%s: %s returning EBUSY state:%d code:%u %s\n",
		      __func__, code_to_string(code), dev->state,
		      dev->ioctl_code, code_to_string(dev->ioctl_code));
		return -EBUSY;
	}
	mtp_log("%s:%u %s\n", __func__, code, code_to_string(code));
	dev->ioctl_code = code;
#else
	if (mtp_lock(&dev->ioctl_excl)) {
...
static long mtp_ioctl(struct file *fp, unsigned int code, unsigned long value)
{
...
#ifdef CONFIG_LGE_USB_GADGET
		if (mtp_lock(&dev->ioctl_excl)) {
			ERROR(dev->cdev, "%s: MTP_SEND_EVENT returning EBUSY code:%u %s\n",
			      __func__,
			      dev->ioctl_code, code_to_string(dev->ioctl_code));
			return -EBUSY;
		}
	...
	case MTP_RECEIVE_CANCEL_EVENT:
		if (mtp_lock(&dev->ioctl_excl)) {
			ERROR(dev->cdev, "%s: MTP_RECEIVE_CANCEL_EVENT returning EBUSY code:%u %s\n",
			      __func__,
			      dev->ioctl_code, code_to_string(dev->ioctl_code));
			return -EBUSY;
		}
...
#ifdef CONFIG_LGE_USB_GADGET
		if (mtp_lock(&dev->ioctl_excl)) {
			ERROR(dev->cdev, "%s: %s returning EBUSY code:%u %s\n",
			      __func__, code_to_string(code),
			      dev->ioctl_code, code_to_string(dev->ioctl_code));
			return -EBUSY;
		}
		mtp_log("%s:%u %s\n", __func__,
